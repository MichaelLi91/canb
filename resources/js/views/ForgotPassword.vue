    ...
    <div class="flex justify-center content-center flex-wrap h-screen w-full bg-purple-700">
        ...
        <form class="w-full" @submit.prevent="ForgotPassword">
          ...
          <div class="text-center md:text-left w-full mb-4">
            <h2 class="text-2xl text-purple-400">パスワードをお忘れですか?</h2>
          </div>
          ...
          <div class="w-full my-3">
           ...
           <router-link class="text-purple-800 float-right" to="/login">戻る</router-link>
          </div>
        </form>
        </div>
    </div>
    ...
    methods: {
        async ForgotPassword(){
            ...
            await this.$store.dispatch('Login',this.user)
                    .then(res=>{
                        this.$store.commit('エラーです．再度リロード後お試しください','パスワードの再設定をメールでお送りいたしました')
                        this.$router.push('/')
                    })
                    .catch(error=>{
                        let errors = [];
